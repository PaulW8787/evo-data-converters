    ...
    def _add_nan_values_metadata(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for attribute_name, values_set in self.nan_values_by_attribute.items():
            if attribute_name in df.columns and values_set:
                df[attribute_name].attrs["nan_values"] = values_set
        return df

    def _apply_nan_values_to_measurements(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Replace sentinel values with None in measurement dataframe.

        Args:
            df: The dataframe containing all measurements
        """
        replacement_dict = {
            col: {sentinel: None for sentinel in sentinels} for col, sentinels in self.nan_values_by_attribute.items()
        }
